...
export default {
  async asyncData({ params }) {
    const latest = await axios.get(`stories/latest`)
    const dailyStories = [latest]
    return { latest, dailyStories }
  },

  data: function () {
    return {
      dailyStories: [this.latest],
      loading: false,
      finished: false
    }
  },
  methods: {
    onLoad: async function () {
      ...
      const daily = await axios.get('stories/before/' + this.dailyStories.at(-1).date)


      this.dailyStories.push(daily)
      this.loading = false;
      if (this.dailyStories.length >= 10) {
        this.finished = true;
      }
    }
  }
