...
router.get("/article/:id", async (ctx, next) => {
  let doc = await articles.findOne({ _id: ObjectId(ctx.request.params.id) });

  ctx.body = {
    ...doc,
    author_res: await users.findOne(
      { _id: ObjectId(doc.author) },
      { projection: { password: 0 } }
    ),
  };
...
router.post("/demo/upimg", upload.single("file"), async (ctx, next) => {
  // console.log(ctx);

  ctx.body = { location: ctx.req.file.filename };
});
