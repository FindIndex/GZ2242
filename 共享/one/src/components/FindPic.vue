<template>
    <div class="picPage">
        <div class="choosedata" @click="showxz = true">
            <div class="data" v-if="!data&&picing">
                {{picing[0]?.maketime.substring(0,4)}}年{{picing[0]?.maketime.substring(5,
                7)}}月</div>
        ...
        <!-- <van-divider v-if="picing" style="margin-top:70rem">{{picing[0]?.maketime.substring(5, 7)}}月</van-divider> -->
        <ul ref="testdiv">
            <FindPicItem v-for="(pic,index) in picing" :key="index" :pic="pic" />
        </ul>
    </div>
</template>

<script>
import FindPicItem from '@/components/items/FindPicItem.vue'
// import FindPicItem from '@/components/items/FindPicItem.vue'
...
export default {
    ...
    methods: {
        ...
        getpic(data) {
            ...
            this.isDao = false
            this.axios
                .get(` https://apis.netstart.cn/one/find/bymonth/0/${data}`)
                .then((res) => {
                    ...
                    this.picing = [...this.picing, ...res.data.data]
                    if (this.picing.length < 7) {
                        // this.prevMonth(data)  
                        this.prevMonth(data)
                        this.getpic(this.yearmonth)
