<template>
    <div class="movie">
        ...
        <ul ref="testdiv">
            ...
            <FindReadItem v-for="(movie,index) in movies" :key="index" :read="movie" />
        </ul>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getmove(data) {
            ...
            this.isDao = false
            this.axios
                .get(` https://apis.netstart.cn/one/find/bymonth/5/${data}`)
                .then((res) => {
                    qing++
                    if (res.data.data.length == 0) {
                        this.prevMonth(data)
                        this.getmove(this.yearmonth);
                    } else {
                        ...
                        this.movies = [...this.movies, ...res.data.data]
        ...
        getNewmove(data) {
            ...
            this.axios
                ...
                .then((res) => {
                    if (res.data.data.length == 0) {
                        this.prevMonth(data)
                        this.getNewmove(this.yearmonth);
                    } else {
                        this.showlocading = true
                        var childDiv1_1 = document.createElement("h2");
                        childDiv1_1.style.textAlign = "center"
